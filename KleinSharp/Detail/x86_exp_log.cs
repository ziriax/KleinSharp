...
namespace KleinSharp
{
	internal static partial class Detail
	{
		...
		public static unsafe void exp(__m128 a, __m128 b, out __m128 p1_out, out __m128 p2_out)
		{
			...
			__m128 a2_sqrt_rcp = rsqrt_nr1(a2);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static unsafe void log(__m128 p1, __m128 p2, out __m128 p1_out, out __m128 p2_out)
		{
			...
			// from the motor.
			__m128 bv_mask = _mm_set_ps(1f, 1f, 1f, 0f);
			...
			bool p_zero = MathF.Abs(p) < 1e-6;
			float u = p_zero ? MathF.Atan2(-q, t_scalar) : MathF.Atan2(s_scalar, p);
